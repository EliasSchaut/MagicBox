...
- `NFC("TEXT", &successNode)` — NFC tag gate (sets `expectedNfc`/`nfcSuccess`, see below). `NFC_WRITE("TEXT")` — write mode: every tag presented while on this node gets TEXT written ("Schreibe … (Chip dranhalten)" / "… geschrieben" over Serial, re-armed per tag; takes priority over the node's own gate/onNfc). `NFC_OFF` is the gap-filler needed when `.onEnter` is set without NFC. The NFC fields sit between `pinSuccess` and `onEnter` in the struct; `onEnter` stays last.
...
- **Story integration** (`story.cpp`): after each transition the resting node arms NFC via `armNfc()` — priority: a write already pending from `onEnter`/`onNfc` is left alone > `nfcWriteText` (declarative write mode: prompt "Schreibe … (Chip dranhalten)", per-tag "… geschrieben", re-armed after every write) > a read iff the node has `expectedNfc` or `onNfc`. On a read, precedence is: `onNfc(text)` fires first (sees every tag, may transition — then the old node's gate is skipped) → `expectedNfc` gate (`strcmp` match → `enterNode(nfcSuccess)`) → no transition → re-arm.
